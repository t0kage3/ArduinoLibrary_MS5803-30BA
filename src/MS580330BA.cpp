...
float MS580330BA::read(){
    ...
    // 温度の補正と結果出力
    uint32_t dt, temperature_uint32;
    dt = (int32_t)d2 - (int32_t)((uint32_t)c[4] << 8);
    temperature_uint32 = 2000 + (int32_t)(((int64_t)dt * (int64_t)c[5]) >> 23);
    temperature = (float)temperature_uint32 / 100;

    // 圧力の補正と結果出力
    uint32_t pressure_uint32;
    uint64_t off, sens;
    off = ((int64_t)c[1] << 16) + (((int64_t)c[3] * dt) >> 7);
    sens = ((int64_t)c[0] << 15) + (((int64_t)c[2] * dt) >> 8);
    pressure_uint32 = (((d1 * sens) >> 21) - off) >> 13;
    pressure = (float)pressure_uint32 / 10;
